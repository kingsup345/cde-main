...
| מדרגת Scalp רגוע | SL 2.3 / TP1 1.8 / TP2 3.5 | `calmRegime.ts` — כל 4 הבוטים |

### מדרגת Scalp רגוע (`calmRegime.ts`, opt-in, 2026-09-11)
בקשת מפעיל: בשוק **רגוע** לסחור מדרגה קבועה — SL 2.3% / TP1 1.8% (50%) /
TP2 3.5% — במקום המדרגה הדינמית (בד"כ רחבה יותר) של כל בוט, כדי לתפוס תנודות
קטנות במקום לרדוף. ב**תנועה גדולה** לא לגעת בכלום.
```
dynSlPct  = הסטופ הדינמי של הבוט עצמו (%)
אם dynSlPct ≥ CALM_SL_THRESHOLD_PCT (2.3)  →  תנועה גדולה: הכל דינמי, כרגיל
אחרת (רגוע)                                →  SL   = 2.3%
                                               TP1  = min(1.8%, TP1 הדינמי)   ← לעולם לא מרחיב יעד
                                               TP2  = 3.5%
                                               שער R:R נמדד מול TP2 (3.5/2.3=1.52 ≥ 1.2), לא TP1 (1.8/2.3=0.78)
```
"תנועה גדולה" מוגדר **זהה בכל בוט**: הסטופ הדינמי **של הבוט עצמו** מול הסף —
לא צריך אינדיקטור תנודתיות חדש, כל בוט כבר מחשב סטופ דינמי. TP1 של 0.78
ביחס-סיכוי-סיכון הוא **כוונה**: פרטיישל מהיר, לא כל התזה — הקצה יושב ברץ
ל-TP2. Pro חסר שער R:R לכן no-op בהיבט הזה (רק המדרגה משתנה).
