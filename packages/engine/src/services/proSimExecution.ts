...
export function applyProEntryGates(
  ...
  return evaluations
    ...
    .map(({ ev }) => {
      ...
      if (ev.indicators?.isDowntrend) {
        // Prevent catching falling knives: block BUY signals generated by oversold indicators during a strong downtrend.
        return gateResult(ev, 'NO_SIGNAL [DOWNTREND_FILTER]', 'מגמת רקע דובית (EMA50 < EMA200), קניית Oversold חסומה למניעת תפיסת סכינים נופלות', false, minConfidence);
      }
      ...
      const spreadPercent = ev.spreadPercent ?? 0;
      // limitEntries: Maker fee (0.02%) + zero slippage. Market: Taker fee (0.10%) + slippage (~0.10%).
      // Add a base cost (0.08%) to account for exit fees (Taker or Maker depending on order).
      const limitEntries = ctx.pending.some(p => p.fill === 'limit') // Approximating limitEntries from context, or pass it in. Since limitEntries isn't in ProGateContext, let's use a conservative estimate.
      const estimatedCost = spreadPercent + 0.20 + 0.08; 
      if (spreadPercent > 0.25 || estimatedCost > 0.45) {
        return gateResult(ev, 'NO_SIGNAL [COST_EXCEEDS_EDGE]', `ספרייד/עלות גבוהים (${estimatedCost.toFixed(2)}%) שוחקים את יעד ה-3%`, false, minConfidence);
      }
